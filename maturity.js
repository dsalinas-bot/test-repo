...
// Updated stage names + richer copy (Characteristics / Risk profile / Mindset)
const STAGES = [
  {
    id: 1,
...
// NEW: structured stage renderer (goes into teaserStageDesc/resultStageDesc)
function renderStageDetails(stage) {
  ...
  const bullets = (items) =>
    (items && items.length)
      ...
      : '';
  const quote = stage.mindset ? `<div class="stage-mindset">>${esc(stage.mindset)}</div>` : '';

  return `
    <div class="stage-core">${esc(stage.core || stage.desc || '')}</div>

    <div class="stage-block">
      <div class="stage-block-title">Characteristics</div>
      ${bullets(stage.characteristics)}
    </div>

    <div class="stage-block">
      <div class="stage-block-title">Risk profile</div>
      ${bullets(stage.risks)}
    </div>
...
function showGate() {
  ...
  // NEW: richer stage copy + stat callout
  const teaserDesc = document.getElementById('teaserStageDesc');
  if (teaserDesc) teaserDesc.innerHTML = renderStageDetails(stage);
  const teaserStatText = document.getElementById('teaserStatText');
  const teaserStatSource = document.getElementById('teaserStatSource');
  if (teaserStatText) teaserStatText.textContent = stage.stat || '';
  if (teaserStatSource) teaserStatSource.textContent = stage.statSource || '';
...
async function submitGate() {
  ...
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    ...
    if (emailErrEl) emailErrEl.textContent = 'Please enter a valid work email.';
...
function selectAnswer(id) {
  ...
  document.querySelectorAll('.answer-btn').forEach(btn => {
    ...
    if (!key) return;
    key.style.background = btn.dataset.id === id ? 'var(--blue)' : '';
    key.style.color = btn.dataset.id === id ? '#fff' : '';
    key.style.borderColor = btn.dataset.id === id ? 'var(--blue)' : '';
  });
...
function renderReview() {
  ...
  if (missing.length > 0) {
    ...
    alert.textContent =
      ...
      `Skipped questions are scored as the lowest level.`;
  } else {
...
function showResults() {
  ...
  // NEW: richer stage copy in results
  const resultDesc = document.getElementById('resultStageDesc');
  if (resultDesc) resultDesc.innerHTML = renderStageDetails(stage);
  ...
  barsEl.innerHTML = pillarSorted.map((p, i) => `
    <div class="pillar-bar-row">
      ...
      <div class="pillar-bar-track">
        <div class="pillar-bar-fill" data-width="${p.percent}"
          style="background:${PILLAR_COLORS[i % PILLAR_COLORS.length]}"></div>
      </div>
    </div>
  `).join('');
  ...
  if (nextStage) {
    ...
    document.getElementById('nextStageDesc').textContent = nextStage.core || nextStage.desc || '';
...
document.addEventListener('keydown', e => {
  ...
  if (['1','2','3','4'].includes(e.key)) {
    const a = q.answers[parseInt(e.key, 10) - 1];
...
function buildShareCard(){
  ...
  const SD = [
    { n:'1', l:'Stage 1', t1:'Every performance expert', t2:'started here.',
      b:['You have a baseline to grow from','Awareness is the first step to improvement','Most teams begin exactly where you are'] },
    { n:'2', l:'Stage 2', t1:"You're building something", t2:'that lasts.',
      b:['Repeatable tests reduce guesswork','Your team shares a common baseline','Performance is becoming part of the process'] },
    { n:'3', l:'Stage 3', t1:'Data drives your decisions.', t2:"That's rare.",
      b:['Testing is baked into your delivery process','Assertions catch regressions before they ship','Dashboards give your team shared visibility'] },
    { n:'4', l:'Stage 4', t1:"You're in the top tier", t2:'of engineering teams.',
      b:['Performance is a continuous practice','You prevent incidents before they happen','Your team forecasts and optimizes proactively'] }
  ];

  const ST = [['Initial','ad hoc'],['Defined','repeatable'],['Managed','measured'],['Optimized','continuous']];
  const sd = SD[si];

  const bg = x.createLinearGradient(0,0,518,988);
  bg.addColorStop(0,'#0A0A0F');
  bg.addColorStop(1,'#0F0F1F');
  x.fillStyle = bg;
  x.fillRect(0,0,W,H);

  const gl = x.createRadialGradient(900,315,0,900,315,720);
  gl.addColorStop(0,'rgba(69,87,221,.25)');
  gl.addColorStop(1,'rgba(69,87,221,0)');
  x.fillStyle = gl;
  x.fillRect(0,0,W,H);

  const ng = x.createLinearGradient(235,359,525,359);
  ng.addColorStop(0,'#4557DD');
  ng.addColorStop(.5,'#F861EE');
  ng.addColorStop(1,'#FF763C');

  x.font = '900 480px Arial Black,Arial,sans-serif';
  x.textAlign = 'left';
  x.textBaseline = 'alphabetic';
  x.fillStyle = ng;
  x.globalAlpha = .9;
  x.fillText(sd.n,220,550);
  x.globalAlpha = 1;

  const dg = x.createLinearGradient(760,0,760,630);
  ...
  x.font='700 10px Inter,Arial,sans-serif';
  x.fillStyle='rgba(255,255,255,.35)';
  x.fillText('YOUR STAGE',822,164);

  const SY=[200,296,380,466];
  SY.forEach((y,i)=>{
    const act=i===si;
    if(i<3){
      x.beginPath();
      x.moveTo(820,y+15);
      x.lineTo(820,SY[i+1]-15);
      x.setLineDash([3,3]);
    ...
    if(act){
      const dg2=x.createRadialGradient(820,y,0,820,y,14);
      dg2.addColorStop(0,'#4557DD');
      dg2.addColorStop(1,'#6C4DD8');
      x.beginPath(); x.arc(820,y,14,0,Math.PI*2); x.fillStyle=dg2; x.fill();
      x.beginPath(); x.arc(820,y,14.75,0,Math.PI*2); x.strokeStyle='#F861EE'; x.lineWidth=1.5; x.stroke();

      x.font='800 11px Inter,Arial,sans-serif';
      x.fillStyle='white';
      x.textAlign='center';
      x.textBaseline='middle';
      x.fillText(i+1,820,y);

      x.textAlign='left';
      x.textBaseline='alphabetic';
      x.font='800 16px Inter,Arial,sans-serif';
      x.fillStyle='white';
      x.fillText(ST[i][0],860,y-2);

      x.font='400 12px Inter,Arial,sans-serif';
      x.fillStyle='rgba(255,255,255,.6)';
      x.fillText(ST[i][1],860,y+16);
    } else {
      x.beginPath(); x.arc(820,y,10,0,Math.PI*2);
      x.strokeStyle='rgba(255,255,255,.2)';
      x.lineWidth=1.5;
      x.stroke();

      x.font='400 10px Inter,Arial,sans-serif';
  ...
  x.textAlign='left';
  x.textBaseline='alphabetic';
  x.fillText('SEE YOUR MATURITY STAGE AT',782,571);

  const ug=x.createLinearGradient(782,594,1156,594);
  ...
  // Keeping Nathan's footer URL text unchanged here
  x.fillText('GATLING.IO/RESOURCES/PERFORMANCE-MATURITY-ANALYZER',782,592);
}
...
function updateLinkedInBtn() {
  ...
  // Option A text
  const text =
    `I just got ${total}/100 in Gatling’s performance maturity assessment. ` +
    `I’m in ${stage.label}. ` +
    `See your score here: ${QUIZ_URL}`;
